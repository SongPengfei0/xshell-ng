...
当前版本是 `0.2.0` 原型阶段，主要面向 Windows 本地使用和开源协作迭代。公开仓库中不应提交真实连接配置、服务器地址、密码、passphrase、私钥或打包产物。
...
- 可在设置中开启自动记录终端日志。
- 日志文件保存在 Windows 文档目录下的 `XShell NG Logs` 文件夹。
...
- `npm run dist:portable`：生成便携版 `release/XShell NG 0.2.0.exe`。
